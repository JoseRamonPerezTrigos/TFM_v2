--io.stderr:write("snort_defaults.lua included\n")
-- To only load this file once to prevent possible overwriting of modified values
if snort_defaults_lua ~= nil then
   return
end
--io.stderr:write("snort_defaults_lua loaded\n")
snort_defaults_lua = true
...
{
    'GET', 'HEAD', 'POST', 'PUT * HTTP/', 'DELETE', 'TRACE', 'CONNECT',
    ...
    "NOTIFY * HTTP/", "OPTIONS * HTTP/", "SUBSCRIBE * HTTP/", 
    "UPDATE * HTTP/", "* * HTTP/"
}

--io.stderr:write("http_methods: ")
--for index,value in pairs(http_methods) do
...
sip_methods =
{
    'INVITE', 'CANCEL', 'BYE', 'REGISTER', 'REFER', 'PUBLISH',
    'JOIN', 'INFO', 'MESSAGE', 'PRACK', "ACK * SIP/", "NOTIFY * SIP/",
    "OPTIONS * SIP/", "SUBSCRIBE * SIP/", "UPDATE * SIP/", "* * SIP/"
}
...
if (TALOS.functions.minsnortver(3,1,7,0,0) or (TALOS.functions.minsnortver(3,1,0,1,174) and TALOS.functions.stopsnortver(3,1,1,0,0))) then
   table.insert(default_wizard.curses, "sslv2")
end
...
if (TALOS.functions.minsnortver(3,1,28,0,0) or (TALOS.functions.minsnortver(3,1,21,1,120) and TALOS.functions.stopsnortver(3,1,22,0,0))) then
   table.insert(default_wizard.curses, "mms")
end

if (TALOS.functions.minsnortver(3,1,44,0,0) or (TALOS.functions.minsnortver(3,1,21,300,3) and TALOS.functions.stopsnortver(3,1,22,0,0)) or (TALOS.functions.minsnortver(3,1,36,1,85) and TALOS.functions.stopsnortver(3,1,37,0,0))) then
   table.insert(default_wizard.curses, "s7commplus")
end
...
default_references =
{
    ...
    { name = 'arachNIDS', url = 'http://obsoletereferencetype.com/404' },
    { name = 'osvdb',     url = 'http://obsoletereferencetype.com/404' },
    { name = 'McAfee',    url = 'http://obsoletereferencetype.com/404' },
    { name = 'nessus',    url = 'http://www.tenable.com/plugins/nessus' },
    { name = 'url',       url = 'http://' },
...
---------------------------------------------------------------------------
-- default whitelist
-- Used to suppress warnings about unrecognized tables when verifying
-- a configuration using --warn-conf-strict
---------------------------------------------------------------------------
default_whitelist =
[[
    ftp_command_specs default_ftp_server smtp_default_alt_max_command_lines
    default_smtp http_methods sip_methods telnet_commands default_wizard default_binder
    ...
    netflow_versions default_js_norm_ident_ignore default_js_norm_prop_ignore
]]
